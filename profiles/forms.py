...
class GameEntryForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if date_started and date_started > date.today():
            self.add_error("date_started", "Start date cannot be in the future.")
        ...
        if date_started and date_completed and date_completed < date_started:
            self.add_error("date_completed", "Completion date cannot be before the start date.")
            return cleaned_data

        # Rule 3: If completed, must have both dates
        if status == "completed":
            if not date_started:
                self.add_error("date_started", "You must enter a start date for a completed game.")
                return cleaned_data
            if not date_completed:
                self.add_error("date_completed", "You must enter a completion date for a completed game.")
                return cleaned_data

        # Rule 4: If playing, cannot have a completion date
        if status == "playing" and date_completed:
            self.add_error("date_completed", "A game you're currently playing cannot have a completion date.")
            return cleaned_data

        # Rule 5: If backlog, no dates allowed
        if status == "backlog" and (date_started or date_completed):
            self.add_error("status", "Backlog games cannot have start or completion dates.")
            return cleaned_data

        # Rule 6: If abandoned, cannot have a completion date
        if status == "abandoned" and date_completed:
            self.add_error("date_completed", "Abandoned games cannot have a completion date.")
            return cleaned_data
